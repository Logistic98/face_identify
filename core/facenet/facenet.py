...
def write_arguments_to_file(args, filename):
    with open(filename, 'w') as f:
        for key, value in vars(args).items():  # correct by zxh
            f.write('%s: %s\n' % (key, str(value)))
...
class TFSession:
    def __init__(self, cuda='0', gpu_mem=0.2, allow_growth=False, is_interactive_session=True):
        ...
        if is_interactive_session:
            self.sess = tf.compat.v1.InteractiveSession(config=tf_config)
        else:
